...
def multiple_run(args):
    ...
    for run in range(args.run_nums):
        ...
        ckpt_path = os.path.join(logger.folder_path, "checkpoints")
        if not os.path.exists(ckpt_path):
            os.makedirs(ckpt_path)
        args.log_path = ckpt_path
        
        buffer = Buffer(args, input_size).cuda()
        model = get_model(method_name=args.method, nclasses=class_num, args=args).cuda()
        ...
        for i in range(len(task_loader)):
            ...
            # acc_list: mean classifier accs from each expert
            # last_tmp_acc: classifier acc from the last expert
            tmp_acc.append(acc_list)
            last_tmp_acc.append(all_acc_list[args.expert])

            # my analysis
            # tmp_acc.append(acc_list)
            # last_tmp_acc.append(all_acc_list['last'])
        ...
        # last_acc_list = all_acc_list['last']
    ...
    print(f"\n{'=' * 100}")
    print(f"total {args.run_nums}runs mean test acc results: {test_all_acc}")
    print('----------- Avg_End_Acc {} Avg_End_Fgt {} Avg_Acc {} Avg_Bwtp {} Avg_Fwt {}-----------'
